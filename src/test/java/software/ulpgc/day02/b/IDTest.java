...
public class IDTest {
    ...
    public void example_test_with_fluid_construction() {
        ...
        assertThat(ClassifierAdder.create().compile(ranges).sum()).isEqualTo(4174379265L);
    }

    @Test
    public void file_test_with_fluid_construction() {
        try (InputStream is = getClass().getResourceAsStream("/day02/b/ranges.txt")) {
            Assert.assertNotNull(is);
            String ranges = new String(is.readAllBytes());
            assertThat(ClassifierAdder.create().compile(ranges).sum()).isEqualTo(28858486244L);
        } catch (IOException e) {
